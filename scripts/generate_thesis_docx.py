...
def add_equation(doc: Document, equation: str, number: str | None = None) -> None:
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.line_spacing = 1.5
    p.paragraph_format.space_before = Pt(0)
    p.paragraph_format.space_after = Pt(0)
    text = equation if number is None else f"{equation}    {number}"
    run = p.add_run(text)
    set_run_font(run, 12)


def add_equation_notes(doc: Document, notes: list[tuple[str, str]]) -> None:
    ...
    run = intro.add_run("　　式中")
    set_run_font(run, 12)

    for symbol, explanation in notes:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        p.paragraph_format.line_spacing = 1.5
        p.paragraph_format.space_before = Pt(0)
        p.paragraph_format.space_after = Pt(0)
        p.paragraph_format.left_indent = Pt(116)
        p.paragraph_format.first_line_indent = Pt(-92)
        p.paragraph_format.tab_stops.add_tab_stop(Pt(92))
        run = p.add_run(f"　　{symbol}\t——{explanation}")
        set_run_font(run, 12)
...
def set_section_page_number_continue(section, fmt: str = "decimal") -> None:
    sect_pr = section._sectPr
    pg_num_type = sect_pr.find(qn("w:pgNumType"))
    if pg_num_type is None:
        pg_num_type = OxmlElement("w:pgNumType")
...
def set_header_footer(section, header_text: str | None, *, roman: bool = False, start: int | None = 1) -> None:
    ...
    if header_text:
        ...
        set_run_font(run, 10)
        set_paragraph_bottom_border(header)

    even_header = section.even_page_header.paragraphs[0]
    even_header.text = ""
    even_header.alignment = WD_ALIGN_PARAGRAPH.CENTER
    even_header.paragraph_format.line_spacing = 1.0
    even_header.paragraph_format.space_before = Pt(0)
    even_header.paragraph_format.space_after = Pt(0)
    even_run = even_header.add_run("重庆邮电大学本科毕业设计（论文）")
    set_run_font(even_run, 10)
    set_paragraph_bottom_border(even_header)

    add_page_number_footer(section)

    if start is not None:
        set_section_page_number_start(section, start, "upperRoman" if roman else "decimal")
    else:
        set_section_page_number_continue(section, "upperRoman" if roman else "decimal")


def set_footer_only(section, *, roman: bool = False, start: int | None = 1) -> None:
...
def add_configured_section(doc: Document, *, header_text: str | None, roman: bool = False, start: int | None = 1) -> None:
    section = doc.add_section(WD_SECTION_START.NEW_PAGE)
    configure_section(section)
    if header_text is None:
        set_footer_only(section, roman=roman, start=start)
    else:
...
def add_abstract(doc: Document) -> None:
    ...
    add_paragraph(
        doc,
        "本文针对低信噪比、带宽受限以及信道波动条件下图像重建质量会下降这一问题，设计并实现了一套基于深度语义通信的图像鲁棒传输系统。系统依赖端到端联合信源信道编码思路，运用卷积神经网络即Convolutional Neural Network，CNN来构建语义编码器以及语义解码器，并且在编码端引入压缩激励即Squeeze-and-Excitation，SE通道注意力以及空间注意力机制，从而把输入图像映射为归一化语义特征；在传输阶段，加入可微加性高斯白噪声即Additive White Gaussian Noise，AWGN信道以及瑞利Rayleigh衰落信道，使模型在训练过程中能够接触噪声以及衰落扰动；接收端则借助转置卷积结构对图像进行恢复。除模型本身之外，系统还实现了联合图像专家小组即Joint Photographic Experts Group，JPEG基线链路、峰值信噪比即Peak Signal-to-Noise Ratio，PSNR以及结构相似性即Structural Similarity，SSIM评价、批量评估脚本以及Streamlit可视化界面，可以完成图像上传、信噪比即Signal-to-Noise Ratio，SNR调节、信道选择，以及原图、深度语义通信即Deep Semantic Communication，DeepSC重构图和传统基线重构图的对比展示。实验选用CIFAR-10来训练模型，并且在Kodak图像集上开展-5 dB至20 dB范围内的蒙特卡洛评估。AWGN信道下，64语义通道模型在-5 dB时取得21.90 dB PSNR和0.6183 SSIM，高于JPEG基线的11.43 dB和0.1099；20 dB时取得40.02 dB PSNR和0.9935 SSIM。Rayleigh信道下，模型在-5 dB时仍达到22.46 dB PSNR和0.6451 SSIM，20 dB时达到38.11 dB PSNR和0.9929 SSIM。上述结果说明，把语义表示学习与可微信道建模置于同一训练闭环当中，可以对噪声以及衰落环境下的图像重建效果进行改善。",
    )
    add_paragraph(doc, "")
    ...
    add_paragraph(
        doc,
        "This thesis addresses the degradation of reconstructed image quality under low signal-to-noise ratio, limited bandwidth, and channel fluctuation by designing and implementing a robust image transmission system based on deep semantic communication. The system relies on an end-to-end joint source-channel coding framework. A convolutional neural network (CNN) is used to construct the semantic encoder and semantic decoder, while squeeze-and-excitation (SE) channel attention and spatial attention are introduced at the encoding side to map the input image into normalized semantic features. During transmission, differentiable additive white Gaussian noise (AWGN) and Rayleigh fading channels are added so that the model can encounter both noise and fading disturbance during training. At the receiving side, the image is recovered through a transposed-convolution-based decoder. In addition to the model itself, the system implements a Joint Photographic Experts Group (JPEG) baseline, peak signal-to-noise ratio (PSNR) and structural similarity (SSIM) evaluation, batch evaluation scripts, and a Streamlit visualization interface. The interface supports image uploading, signal-to-noise ratio (SNR) adjustment, channel selection, and comparison among the original image, the deep semantic communication (DeepSC) reconstruction, and the traditional baseline reconstruction. CIFAR-10 is used for model training, and Monte Carlo evaluation is carried out on the Kodak image set over the range from -5 dB to 20 dB. Under the AWGN channel, the 64-semantic-channel model achieves 21.90 dB PSNR and 0.6183 SSIM at -5 dB, which are higher than the JPEG baseline results of 11.43 dB and 0.1099. At 20 dB, it achieves 40.02 dB PSNR and 0.9935 SSIM. Under the Rayleigh channel, the model still reaches 22.46 dB PSNR and 0.6451 SSIM at -5 dB, and reaches 38.11 dB PSNR and 0.9929 SSIM at 20 dB. These results show that placing semantic representation learning and differentiable channel modeling in the same training loop can improve image reconstruction under noisy and fading environments.",
    )
    add_paragraph(doc, "")
...
def add_table_of_contents(doc: Document) -> None:
    set_header_footer(doc.sections[-1], "目录", roman=True, start=None)
...
def add_chapter_1(doc: Document) -> None:
    ...
    add_paragraph(doc, "国外关于无线图像语义传输的研究多从深度联合信源信道编码切入。E. Bourtsoulatze等人提出DeepJSCC方法，直接学习从图像到信道输入符号的映射，并用端到端训练完成重建[12]。与传统压缩和纠错分开设计的方案相比，这类方法在低信噪比区域通常不会突然失效，而是随信道恶化逐步下降。Z. Qin等人从语义通信的基本问题出发讨论了含义传递、知识共享和评价标准[13]；H. Xie等人提出DeepSC文本语义通信系统[14]，其中的语义编码、信道扰动和接收端恢复思路，对图像任务也有参考价值。")
    add_paragraph(doc, "后续工作把重点放在带宽自适应、网络结构改进和信道鲁棒性上。DeepJSCC-l++针对不同带宽和信道条件调整传输特征[15]；WITT将Transformer用于无线图像传输[16]；SwinJSCC则把Swin Transformer结构引入深度JSCC框架[17]。另一些研究关注未知信道、未知数据分布和对抗扰动下的语义恢复稳定性[18-19]。这些工作使用的模型普遍更复杂，训练成本也更高。本文没有直接复现大型Transformer方案，而是选择卷积注意力结构，是为了在本科设计范围内把模型训练、信道仿真、指标评估和界面演示完整串起来。")
    add_paragraph(doc, "这些研究对本文的设计影响主要体现在两个方面。一是信道扰动必须进入训练过程，否则模型容易退化成普通图像自编码器；二是实验不能只展示单张图像或单个SNR点，至少需要在多个SNR条件和多张测试图像上统计平均结果。本文后续的训练策略、Kodak评估和蒙特卡洛采样均按这一思路安排。")
...
def add_chapter_2(doc: Document) -> None:
    ...
    add_equation_notes(
        ...
        [
            ("x_hat", "接收端重建图像。"),
            ("g_phi", "语义解码器函数，phi为解码器参数。"),
            ("H", "信道扰动函数。"),
            ("f_theta", "语义编码器函数，theta为编码器参数。"),
            ("x", "输入原始图像。"),
        ],
    )
    add_paragraph(doc, "模型训练的目标是最小化原图x与重建图x_hat之间的差异，通常可写为：")
    ...
    add_paragraph(doc, "本章围绕图像语义通信系统所需的基础理论展开说明。首先介绍深度语义通信的基本思想，说明本文中的DeepSC并不是单一固定网络名称，而是由语义编码、信道扰动和语义解码共同组成的端到端传输框架；随后对联合信源信道编码与传统分离式图像传输进行了对比，指出端到端联合优化在低SNR场景下具有更平滑的退化特性；最后给出AWGN信道、Rayleigh信道以及MSE、PSNR、SSIM等评价指标的定义和作用。上述内容为后续系统需求分析、模型结构设计和实验结果解释提供了理论依据。")
...
def add_chapter_3(doc: Document) -> None:
    ...
    add_paragraph(doc, "本章从毕业设计任务目标出发，对图像鲁棒传输系统的功能性需求和非功能性需求进行了分析，并给出了系统总体架构与模块划分。系统整体由输入层、核心处理层、支撑层和展示层组成，核心处理层负责DeepSC模型和JPEG基线，支撑层负责信道仿真、指标评估和实验产物管理，展示层通过Streamlit界面完成交互式演示。通过本章设计，可以明确论文后续实现工作所对应的工程边界，也为第4章展开模型、信道、评估和可视化模块实现奠定基础。")
...
def add_chapter_4(doc: Document) -> None:
    ...
    add_equation_notes(
        ...
        [
            ("L", "混合训练损失函数。"),
            ("alpha", "SSIM损失权重，当前配置中取0.2。"),
            ("MSE", "均方误差项，用于约束像素级误差。"),
            ("x", "输入原始图像。"),
            ("x_hat", "模型重建图像。"),
            ("SSIM", "结构相似性项，用于约束图像结构保持能力。"),
        ],
    )
    add_paragraph(doc, "MSE与SSIM结合能够在清晰度和结构保持之间取得较好的平衡。优化器采用自适应动量估计（Adaptive Moment Estimation，Adam）的带权重衰减变体（Adam with Weight Decay，AdamW），学习率为0.001，权重衰减为0.0001。")
    ...
    add_paragraph(doc, "本章对系统关键模块的设计与实现进行了说明。模型部分采用卷积编码器、注意力模块、功率归一化、可微信道层和转置卷积解码器组成端到端图像语义传输链路；信道部分实现AWGN和Rayleigh两类典型扰动，并通过混合损失函数约束像素误差和结构相似性；工程实现部分完成JPEG基线、批量评估、单图推理、交互式参数配置、checkpoint安全加载和Streamlit可视化界面。上述模块共同构成了可训练、可评估、可演示的系统原型，为第5章开展定量实验和可视化分析提供了实现基础。")
...
def add_chapter_5(doc: Document) -> None:
    ...
    add_paragraph(doc, "AWGN信道下，64语义通道DeepSC模型在各个测试SNR点都高于JPEG基线。差距在低信噪比处最明显：-5 dB时DeepSC的PSNR为21.90 dB，JPEG基线为11.43 dB；SSIM分别为0.6183和0.1099。此时JPEG重构图已经出现明显退化，而DeepSC仍能保留主要轮廓和区域结构。SNR提高后，两种方法的指标都会上升，但DeepSC在20 dB时达到40.02 dB PSNR和0.9935 SSIM，说明高信噪比下也没有因为语义瓶颈造成明显质量损失。")
    add_paragraph(doc, "图5.3和图5.4给出了同一组结果的曲线形式。JPEG基线在-5 dB和0 dB时PSNR分别约为11.43 dB和15.46 dB，SSIM仅为0.1099和0.2111，强噪声会直接破坏压缩图像的结构。DeepSC在-5 dB时PSNR仍超过20 dB，在0 dB时达到29.73 dB和0.8899 SSIM。这个变化更接近DeepJSCC类方法常见的平滑退化特性[12,15]：信道变差时质量下降，但不会在某个阈值附近突然崩溃。")
    add_paragraph(doc, "从具体数值看，AWGN信道下DeepSC的优势并不只来自某一个测试点。低SNR区域体现抗噪能力，高SNR区域则能观察语义瓶颈是否限制细节恢复。64语义通道在20 dB时仍有较高SSIM，说明该通道宽度足以承载Kodak图像中的主要结构和大部分纹理信息。JPEG基线在高SNR下也会恢复，但低SNR处的失真已经影响轮廓和块状区域，因此平均曲线被明显拉低。")
    ...
    add_paragraph(doc, "Rayleigh信道引入随机幅度衰落，比单纯AWGN更接近无线链路中的不稳定情况。表5.3中，DeepSC在-5 dB时取得22.46 dB PSNR和0.6451 SSIM，JPEG基线为11.86 dB和0.1484；20 dB时DeepSC为38.11 dB和0.9929。与AWGN结果相比，高SNR处PSNR略低，说明衰落仍会增加重建难度，但SSIM保持得较好，图像的主要结构没有明显被破坏。")
    add_paragraph(doc, "图5.5和图5.6也能看到这一点。JPEG基线在Rayleigh信道下恢复较慢，20 dB时PSNR仍低于18 dB，视觉质量受衰落影响较大。DeepSC在5 dB后SSIM超过0.96，10 dB后接近0.985，说明模型学到的语义特征对乘性衰落有一定适应性。结合AWGN实验可以看出，本文模型并不是只对单一加性噪声有效，在引入衰落后仍能保持较稳定的结构恢复效果，这与鲁棒语义通信研究中强调的跨信道稳定性一致[10,18]。")
    add_paragraph(doc, "需要注意的是，本文采用的Rayleigh信道仍是简化模型，并且接收端使用理想均衡，因此实验结果不能直接等同于真实移动信道中的表现。它的作用主要是引入比AWGN更强的随机扰动，用来检验语义特征在幅度变化后的恢复情况。后续若加入信道估计误差和频率选择性衰落，PSNR可能会继续下降，但当前结果已经能够说明模型在两类典型信道下的基本鲁棒性。")
    ...
    add_paragraph(doc, "本章围绕系统训练、测试和实验结果进行了分析。实验以CIFAR-10作为训练数据，以Kodak图像集作为测试数据，在AWGN和Rayleigh信道下分别比较DeepSC模型与JPEG基线的PSNR、SSIM和可视化效果。结果表明，64语义通道模型在低SNR条件下仍能保持较好的结构恢复能力，Rayleigh衰落环境下也没有出现明显失效；不同语义通道宽度的对比说明，16、32和64通道分别对应压缩优先、折中和质量优先三类配置。与此同时，本章也说明了训练数据分辨率、JPEG基线设置、简化信道模型和评价指标选择带来的局限。总体来看，实验结果能够支撑本文关于深度语义通信改善噪声与衰落环境下图像重建质量的主要判断。")


def add_chapter_6(doc: Document) -> None:
    add_heading(doc, "第6章 总结与展望", 1)
    add_heading(doc, "6.1 研究工作总结", 2)
    add_paragraph(doc, "本文完成了一个可训练、可评估、可演示的图像语义传输系统。论文先梳理语义通信、联合信源信道编码、无线信道和图像质量评价等内容，再围绕图像重建任务设计DeepSC模型。模型由卷积注意力编码器、可微信道层和转置卷积解码器组成，训练时把AWGN和Rayleigh信道放入前向传播过程，使编码器和解码器在有扰动的条件下共同优化。")
    add_paragraph(doc, "工程实现部分没有只停留在模型脚本上。训练程序可以按配置批量训练不同语义通道数模型，并保存checkpoint、训练历史、损失曲线和summary文件；评估程序可以在Kodak图像集上进行多SNR、多蒙特卡洛采样测试，输出指标表和曲线；单图推理程序可以生成DeepSC重构图、JPEG基线图和对应指标；Streamlit界面则把图像上传、checkpoint选择、信道切换和指标展示放到同一交互流程中。")
    add_paragraph(doc, "实验结果主要有两点。其一，在AWGN和Rayleigh信道的低SNR区域，DeepSC模型明显优于JPEG基线，低信噪比下仍能保留图像轮廓和主要结构。其二，语义通道数会直接影响压缩率和重建质量：16通道更偏向压缩，64通道更偏向质量，32通道处在两者之间。这个结果也说明，语义瓶颈宽度不能只按模型规模选择，还要结合带宽开销和应用场景一起考虑。")
    ...
    add_paragraph(doc, "后续工作可以先从模型和信道两端继续完善。模型方面，可尝试多尺度CNN、Swin Transformer或CNN-Transformer混合结构，以改善大尺寸图像中的长距离依赖和细纹理恢复。信道方面，当前系统只覆盖AWGN和简化Rayleigh信道，后续还应加入Rician衰落、多径时变信道、量化噪声、调制误差和信道估计误差，让训练环境更接近真实无线链路。评价方法也需要继续补充。PSNR和SSIM便于复现，但它们不能完整反映主观视觉质量和任务可用性，后续可加入LPIPS、FID或目标检测、分类等下游任务指标。系统实现层面，还可以继续压缩模型、优化推理速度，并将当前Streamlit演示扩展为Web服务或边缘端部署版本。")
    add_paragraph(doc, "数据集和分辨率也是下一步需要处理的问题。本文训练主要使用CIFAR-10，图像尺寸较小，虽然测试阶段使用Kodak图像集验证了尺寸泛化，但模型没有在大规模高清自然图像上长期训练。后续可以使用更高分辨率数据集重新训练，并比较不同语义通道宽度在高清图像上的带宽开销和恢复细节。这样得到的结论会更接近实际图像业务，也能更清楚地判断本文轻量模型的上限。")
...
def main() -> None:
    ...
    add_configured_section(doc, header_text=None, roman=True, start=1)
    add_abstract(doc)
    add_configured_section(doc, header_text="目录", roman=True, start=None)
